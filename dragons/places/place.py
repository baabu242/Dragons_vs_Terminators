...
class Place(object):
    ...
    def add_fighter(self, fighter):
        ...
        if fighter.is_dragon:
            if self.dragon is None:
                ...
                # BEGIN 3.2
                if self.dragon.is_container is False and fighter.is_container is True :
                    fighter.contain_dragon(self.dragon)
                    self.dragon = fighter
                elif self.dragon.is_container is True and self.dragon.can_contain(fighter) is True:
    ...
    def remove_fighter(self, fighter):
        ...
        if fighter.is_dragon:
            ...
            # BEGIN 4.3
            "*** YOUR CODE HERE ***"
            if fighter.dking is True :
                if fighter.king is True :
                    fighter.place = self
                    return
                if self.dragon is fighter :
                    if fighter.king is False :
                        self.dragon = None
                        fighter.place = None
                elif hasattr(self.dragon, 'is_container') and self.dragon.is_container \
